...
local ____exports = {}
local CDS_GetHGCfg, CDS_NearestLaneToLocation, updateGameStateCache, updateLocationStateCache, updateUnitStateCache, presence_adjust, Customize, StartToPushTime, BOT_MODE_DESIRE_EXTRA_LOW, hEnemyAncient, PUSH_CACHE_TTL, gameStateCache, locationStateCache, unitStateCache, BASE_ANC_RADIUS
...
function CDS_NearestLaneToLocation(nTeam, vLoc)
    local best = nil
...
function ____exports.GetPushDesireHelper(bot, lane)
    ...
    local ____cdsCfg_Ping_Radius_2 = cdsCfg.Ping_Radius
    if ____cdsCfg_Ping_Radius_2 == nil then
        ____cdsCfg_Ping_Radius_2 = 1600
    end
    local nPingRadius = ____cdsCfg_Ping_Radius_2
    local ____cdsCfg_Ping_Memory_Seconds_3 = cdsCfg.Ping_Memory_Seconds
    if ____cdsCfg_Ping_Memory_Seconds_3 == nil then
        ____cdsCfg_Ping_Memory_Seconds_3 = 30
    end
    local nPingMemory = ____cdsCfg_Ping_Memory_Seconds_3
    local ____cdsCfg_Force_Close_Minute_4 = cdsCfg.Force_Close_Minute
    if ____cdsCfg_Force_Close_Minute_4 == nil then
        ____cdsCfg_Force_Close_Minute_4 = 30
    end
    local nForceCloseMin = ____cdsCfg_Force_Close_Minute_4
    local cdsHuman, cdsPing = jmz.GetHumanPing()
    if cdsHuman ~= nil and cdsPing ~= nil and cdsPing.location ~= nil and not cdsPing.normal_ping and DotaTime() > 0 and GameTime() < cdsPing.time + nPingMemory then
        ...
        if isPinged then
            ...
            local cdsEnemyFountain = gameState.team == Team.Radiant and DireFountainTpPoint or RadiantFountainTpPoint
            if GetLocationToLocationDistance(cdsPing.location, cdsEnemyFountain) < 9000 then
                if CDS_NearestLaneToLocation(
                    GetOpposingTeam(),
                    cdsPing.location
                ) == lane then
                    return 0.9
    ...
    local bForceClose = nForceCloseMin > 0 and gameState.currentTime > nForceCloseMin * 60 and gameState.aliveAllyCount >= gameState.aliveEnemyCount and gameState.aliveAllyCount >= 4
    ...
    if GetLocationToLocationDistance(laneFront, enemyFountain) < 5000 then
        local alliesAtFront = getCachedAlliesNearLoc(laneFront, 2500)
        local bManpower = #alliesAtFront >= 3 or #alliesHere >= 3
        local bEnemiesDown = gameState.aliveEnemyCount <= gameState.aliveAllyCount - 2
        if not (bManpower or bEnemiesDown or gameState.hasAegis or bForceClose) or gameState.aliveAllyCount < gameState.aliveEnemyCount then
    ...
    if not jmz.IsCore(bot) and isCurrentLanePushLane or jmz.IsCore(bot) and (jmz.IsLateGame() and isCurrentLanePushLane or isMidOrEarlyGame) then
        local allowNumbers = bForceClose or eAliveCount == 0 or aAliveCoreCount >= eAliveCoreCount or aAliveCoreCount >= 1 and aAliveCount >= eAliveCount + 2 or networthAdvantage > 8000 and aAliveCount >= eAliveCount - 1 or levelAdvantage > 2 and aAliveCount >= eAliveCount - 1
        if allowNumbers then
            ...
            if bForceClose and jmz.GetHP(bot) > 0.5 then

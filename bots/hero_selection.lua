...
local function CorrectPotentialLaneAssignment()
	--[[ CDS-PATCH: keep slot i's role paired with slot i's lane.
	     The old Dire branch read the role at player-slot `i` but wrote the lane at a
	     separate humans-first cursor `index`, permuting roles against lanes whenever
	     a human sat anywhere but slot 1. With PlayerID-keyed output (see
	     ToPlayerIdKeyedLanes) the reordering is unnecessary, so both teams now use
	     a straight slot-for-slot mapping and ipairs for deterministic order. ]]
	if GetTeam() == TEAM_RADIANT and not CorrectRadiantAssignedLanes then
		...
		for i = 1, #players do
			...
			if role ~= nil and tDefaultLaningRadiant[role] ~= nil then
	...
	elseif GetTeam() == TEAM_DIRE and not CorrectDireAssignedLanes then
		local players = GetTeamPlayers(TEAM_DIRE)
		for i = 1, #players do
			local role = Role.RoleAssignment['TEAM_DIRE'][i]
			if role ~= nil and tDefaultLaningDire[role] ~= nil then
				tLaneAssignList.TEAM_DIRE[i] = tDefaultLaningDire[role]
			end
			CorrectDirePlayerIndexToLaneIndex[i] = i
		end
...
function UpdateLaneAssignments()
	...
	if GetGameMode() == GAMEMODE_MO then
		...
		return ToPlayerIdKeyedLanes(MidOnlyLaneAssignment)
	end
	if GetGameMode() == GAMEMODE_1V1MID then
		return ToPlayerIdKeyedLanes(OneVoneLaneAssignment)
	end
	...
	return ToPlayerIdKeyedLanes(tLaneAssignList[team])
end

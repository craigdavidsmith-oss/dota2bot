...
--[[ ==========================================================================
     CDS-PATCH: rune unpredictability.

     Stock behaviour picks the geometrically closest living ally for every rune,
     every time, which makes the opening two rune runs identical in every game.

     Every bot on a team shares this Lua state (mode files are per-bot closures but
     `require`d modules are shared), yet we still want each bot to reach the SAME
     conclusion without ordering races. So instead of rolling dice per bot we
     derive a deterministic hash from (rune, playerId, rune window). Every bot
     computes the same jitter for every teammate, the assignment stays coherent,
     and it changes from window to window and game to game.
     ========================================================================== ]]

local RuneCfg = (Customize.Enable and Customize.Runes) or {}
local nRuneJitter        = RuneCfg.Unpredictability or 0
local nRuneContestChance = RuneCfg.Contest_Chance or 0
local bRuneAllowSmoke    = RuneCfg.Allow_Smoke_Contest ~= false

-- Per-match salt so two games with identical hero picks do not play out identically.
-- Seeded lazily: the bot API is available at file-load time, but being defensive
-- here costs nothing and avoids a load-order surprise.
local nRuneSalt = nil
local function RuneSalt()
	if nRuneSalt == nil then nRuneSalt = RandomInt(1, 99991) end
	return nRuneSalt
end

-- djb2-style. Deliberately arithmetic-only: Dota's bot VM is Lua 5.1, which has
-- no bitwise operators, so `~`/`&`/`|` would be a syntax error here.
local function RuneHash(a, b, c)
	local h = 5381
	local vals = { a or 0, b or 0, c or 0, RuneSalt() }
	for i = 1, #vals do
...
-- 0..1, stable for a given (rune, player, window) triple.
local function RuneUnit(a, b, c)
	return (RuneHash(a, b, c) % 10007) / 10007
end

-- Which rune window are we in? Bounties/power runes are on 2-3 minute cadences;
-- a 1 minute bucket is granular enough and keeps every bot in the same bucket.
function X.GetRuneWindow()
	return math.floor(math.max(DotaTime(), 0) / 60)
end

-- Multiplier applied to a hero's distance to a rune. 1.0 with jitter disabled.
function X.GetRuneBias(hUnit, nRune)
	if nRuneJitter <= 0 or not J.IsValidHero(hUnit) then return 1.0 end
	local u = RuneUnit(nRune, hUnit:GetPlayerID(), X.GetRuneWindow())
	return 1.0 + (u * 2.0 - 1.0) * nRuneJitter
end

function X.GetBiasedDistance(hUnit, vLocation, nRune)
	return GetUnitToLocationDistance(hUnit, vLocation) * X.GetRuneBias(hUnit, nRune)
end

-- Is this window a "contest" window, and if so which rune are we contesting?
-- Returns nil when we are not contesting anything.
function X.GetContestRune()
	if nRuneContestChance <= 0 then return nil end
	local w = X.GetRuneWindow()
	if RuneUnit(9001, 0, w) >= nRuneContestChance then return nil end

	-- Prefer a power rune on the far side of the river, else an enemy-side bounty.
	local candidates = {}
	for _, rune in pairs(nRuneList) do
		local vLoc = GetRuneSpawnLocation(rune)
		if vLoc ~= nil and not X.IsMissing(rune) then
			-- "Far" = closer to the enemy fountain than to ours.
			local dOurs   = J.GetDistance(vLoc, GetAncient(GetTeam()):GetLocation())
			local dTheirs = J.GetDistance(vLoc, GetAncient(GetOpposingTeam()):GetLocation())
			if dTheirs < dOurs then table.insert(candidates, rune) end
		end
	end
	if #candidates == 0 then return nil end
	return candidates[(RuneHash(4242, 0, w) % #candidates) + 1]
end

-- The two heroes assigned to a contest run: the two least-biased-distance allies.
function X.IsAssignedToContest(hUnit, nRune)
	local vLoc = GetRuneSpawnLocation(nRune)
	if vLoc == nil then return false end
	local ranked = {}
	for i = 1, 5 do
		local member = GetTeamMember(i)
		if J.IsValidHero(member) and member:IsAlive() and member:IsBot() then
			table.insert(ranked, { u = member, d = X.GetBiasedDistance(member, vLoc, nRune) })
		end
	end
	table.sort(ranked, function(x, y)
		if x.d == y.d then return x.u:GetPlayerID() < y.u:GetPlayerID() end
		return x.d < y.d
	end)
	for i = 1, math.min(2, #ranked) do
		if ranked[i].u == hUnit then return true end
	end
	return false
end
...
function Think()
	...
	-- CDS-PATCH: pop smoke before a contested rune run so the gank is not telegraphed.
	if bRuneAllowSmoke and bot.contestingRune ~= nil and not bot:HasModifier('modifier_smoke_of_deceit') then
		local hSmoke = J.IsItemAvailable('item_smoke_of_deceit')
		if J.CanCastAbility(hSmoke) then
			local vRuneLoc = GetRuneSpawnLocation(bot.contestingRune)
			if vRuneLoc ~= nil
			and GetUnitToLocationDistance(bot, vRuneLoc) > 2200
			and #J.GetAlliesNearLoc(bot:GetLocation(), 1000) >= 2
			and #J.GetEnemiesNearLoc(bot:GetLocation(), 1600) == 0
			then
				bot:Action_UseAbility(hSmoke)
				return
			end
		end
	end
...
function X.GetBestRune()
	...
	-- CDS-PATCH: contest plan takes priority for the two assigned heroes.
	local nContestRune = X.GetContestRune()
	if nContestRune ~= nil
	and not X.IsPingedByHumanPlayer(GetRuneSpawnLocation(nContestRune), math.huge)
	and not IsHumanClaimingRune(nContestRune)
	and X.IsAssignedToContest(bot, nContestRune)
	then
		bot.contestingRune = nContestRune
		return nContestRune, GetUnitToLocationDistance(bot, GetRuneSpawnLocation(nContestRune))
	end
	bot.contestingRune = nil

	for _, rune in pairs(nRuneList) do
		local vRuneLocation = GetRuneSpawnLocation(rune)

		if rune ~= nContestRune
		and X.IsTheClosestAlly(bot, vRuneLocation, rune)
		...
		then
			...
			then
				-- CDS-PATCH: biased distance so the same hero does not always win.
				local dist = X.GetBiasedDistance(bot, vRuneLocation, rune)
...
--------------------------------------------------------------------
function X.IsTheClosestAlly(hUnit, vLocation, nRune)
	-- CDS-PATCH: nRune is optional. When supplied, distances are biased by the
	-- per-window hash so every bot still agrees on a single winner, but the
	-- winner is not always the same hero.
	local targetAlly = hUnit
	local targetAllyDistance = nRune and X.GetBiasedDistance(hUnit, vLocation, nRune)
		or GetUnitToLocationDistance(hUnit, vLocation)
	for i = 1, 5 do
		local member = GetTeamMember(i)
		if J.IsValidHero(member) then
			local memberDistance = nRune and X.GetBiasedDistance(member, vLocation, nRune)
				or GetUnitToLocationDistance(member, vLocation)

...
function GetDesireHelper()
	...
	--[[ CDS-PATCH: the other half of the high-ground fix. Raising push desire is not
	     enough on its own if farm desire is still competitive, so once the closeout
	     window opens for an even-or-ahead team, jungling is pushed down to noise. ]]
	local HGCfg = (Customize.Enable and Customize.HighGround) or {}
	local nForceCloseMin = HGCfg.Force_Close_Minute or 30
	if nForceCloseMin > 0
	and DotaTime() > nForceCloseMin * 60
	and nAliveAllyCount >= 4
	and nAliveAllyCount >= nAliveEnemyCount
	then
		nFarmCap = Min(nFarmCap, 0.15)
	end
	...
	then
		...
		then
			...
			if preferedCamp ~= nil then
				--[[ CDS-PATCH: a pos 4/5 must not open a camp that one of our cores is
				     sitting on or walking to. The stock check below only fires once the
				     ally is already in FARM mode, which is too late -- by then the
				     support has usually taken the big creep. ]]
				local SupportCfg = (Customize.Enable and Customize.Support_No_Steal) or {}
				if SupportCfg.Enable == true and J.GetPosition(bot) >= 4 then
					local nCampCoreRadius = SupportCfg.Camp_Core_Radius or 1600
					local nCoresAtCamp = J.GetAlliesNearLoc(preferedCamp.cattr.location, nCampCoreRadius)
					for _, ally in pairs(nCoresAtCamp) do

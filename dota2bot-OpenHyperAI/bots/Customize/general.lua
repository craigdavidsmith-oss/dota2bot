...
--[[ CDS-PATCH: behaviour tuning added by this fork. All of these are safe to
     leave at their defaults; set Unpredictability to 0 to get stock behaviour. ]]

-- Rune behaviour.
Customize.Runes = {
    -- 0 = always send the geometrically closest hero (stock, fully deterministic).
    -- 0.35 is a good "still sensible, no longer scripted" value. Max 1.0.
    Unpredictability = 0.35,

    -- Chance per rune window that the team deliberately contests a rune it would
    -- normally concede (the far power rune / an enemy-side bounty) with two heroes.
    Contest_Chance = 0.25,

    -- Allow bots on a contest run to pop Smoke of Deceit on the way.
    Allow_Smoke_Contest = true,
}

-- Stop pos 4 / pos 5 bots taking creeps and neutral camps off their own cores.
Customize.Support_No_Steal = {
    Enable = true,
    -- A core inside this radius means the support keeps its hands off the creeps.
    -- Stock value was 700, which is inside melee creep aggro range.
    Core_Radius = 1200,
    -- Supports will not start a neutral camp with an ally core this close to it.
    Camp_Core_Radius = 1600,
}

-- Late-game closing behaviour.
Customize.HighGround = {
    -- After this many minutes, a team that is ahead / even stops jungling and
    -- commits to high ground instead of farming indefinitely. 0 disables.
    Force_Close_Minute = 30,
    -- How long a human ping stays "live" for push/defend decisions, in seconds.
    -- Stock is 5, which is why bots seem to ignore pings.
    Ping_Memory_Seconds = 30,
    -- How far from a pinged objective the ping still counts. Stock is 700.
    Ping_Radius = 1600,
}

-- Compensation for the Dire side underperforming Radiant.
-- Applied on top of the Fretbots bonus for Dire bots only. 1.0 = no change.
-- 1.15 - 1.3 is a reasonable range. Only has an effect in Fretbots mode.
Customize.Dire_Bonus_Scale = 1.0

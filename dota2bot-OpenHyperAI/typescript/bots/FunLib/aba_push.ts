 ...
 */
/* CDS-PATCH: pingTimeDelta removed -- ping lifetime is now Ping_Memory_Seconds. */
const StartToPushTime = 16 * 60; // after X mins, start considering to push.
const BOT_MODE_DESIRE_EXTRA_LOW = 0.02;

/* CDS-PATCH: high-ground commitment tunables, read from Customize/general.lua. */
function CDS_GetHGCfg(): any {
    if (!Customize.Enable) return {};
    return (Customize as any).HighGround || {};
}

/** Which lane front is this location closest to? Lets a human ping that is not
 *  sitting exactly on a building still map to a lane. */
function CDS_NearestLaneToLocation(nTeam: Team, vLoc: Vector): Lane | null {
...
export function GetPushDesireHelper(bot: Unit, lane: Lane): BotModeDesire {
    ...
    /* CDS-PATCH: honour human pings BEFORE the safety gates.
       Stock code checked the ping ~40 lines further down, after several hard
       `return None` gates, so a ping could never override "don't push while
       outnumbered" -- exactly the situation you ping in. It also required the ping
       to land within 700 units of a visible enemy building and be under 5 seconds
       old, hence "bots ignore pings unless they're right there". */
    const cdsCfg = CDS_GetHGCfg();
    const nPingRadius: number = cdsCfg.Ping_Radius ?? 1600;
    const nPingMemory: number = cdsCfg.Ping_Memory_Seconds ?? 30;
    const nForceCloseMin: number = cdsCfg.Force_Close_Minute ?? 30;

    const [cdsHuman, cdsPing] = jmz.GetHumanPing();
    if (cdsHuman !== null && cdsPing !== null && cdsPing.location !== null && !cdsPing.normal_ping && DotaTime() > 0 && GameTime() < cdsPing.time + nPingMemory) {
        const [isPinged, pingedLane] = jmz.IsPingCloseToValidTower(GetOpposingTeam(), cdsPing, nPingRadius, nPingMemory);
        if (isPinged) {
            if (lane === pingedLane) return 0.95 as BotModeDesire;
        } else {
            const cdsEnemyFountain = gameState.team === Team.Radiant ? DireFountainTpPoint : RadiantFountainTpPoint;
            if (GetLocationToLocationDistance(cdsPing.location, cdsEnemyFountain) < 9000) {
                if (CDS_NearestLaneToLocation(GetOpposingTeam(), cdsPing.location) === lane) {
    ...
    /* CDS-PATCH: past this minute, an even-or-ahead team closes the game out
       instead of looping the jungle forever. */
    const bForceClose = nForceCloseMin > 0 && gameState.currentTime > nForceCloseMin * 60 && gameState.aliveAllyCount >= gameState.aliveEnemyCount && gameState.aliveAllyCount >= 4;
    ...
    if (GetLocationToLocationDistance(laneFront, enemyFountain) < 5000) {
        /* CDS-PATCH: the stock test asked for three allies within 1600 of THE BOT,
           which is a grouping test, not a high-ground test -- and clamped desire to
           0.08, so nobody ever walked up the ramp. Count allies at the lane front
           instead, and give credit for dead enemies, aegis, or closeout. */
        const alliesAtFront = getCachedAlliesNearLoc(laneFront, 2500);
        const bManpower = alliesAtFront.length >= 3 || alliesHere.length >= 3;
        const bEnemiesDown = gameState.aliveEnemyCount <= gameState.aliveAllyCount - 2;
        if (!(bManpower || bEnemiesDown || gameState.hasAegis || bForceClose) || gameState.aliveAllyCount < gameState.aliveEnemyCount) {
    ...
    // CDS-PATCH: the ping check that used to live here has been hoisted to the top
    // of this function so it can outrank the hard safety gates above.
    ...
    if ((!jmz.IsCore(bot) && isCurrentLanePushLane) || (jmz.IsCore(bot) && ((jmz.IsLateGame() && isCurrentLanePushLane) || isMidOrEarlyGame))) {
        ...
        if (allowNumbers) {
            ...
            /* CDS-PATCH: in closeout, lift the ceiling above farm/roam desires so the
               team actually commits instead of drifting back to the jungle. */
            if (bForceClose && jmz.GetHP(bot) > 0.5) {
                nMaxDesire = Math.max(nMaxDesire, 0.95);
                nPushDesire = Math.max(nPushDesire, 0.9);
            }
